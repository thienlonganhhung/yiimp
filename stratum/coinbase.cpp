...
//			debuglog("IMG: (coind->p2sh_address && !key) pack tx %s\n", data);
	...
	if(strcmp(coind->symbol, "IMG") == 0){
		sprintf(data+strlen(data), "%s", "00"); // "52696173" - "Rias"

//		debuglog("IMG: (data) pack tx %s\n", data+ol);
//		debuglog("IMG: (ammount) pack tx %lld\n", amount);
	}
}

static void job_pack_tx_IMG(YAAMP_COIND *coind, char *data, json_int_t amount, char *key)
{
	int ol = strlen(data);
	char evalue[32];
	...
	sprintf(data+strlen(data), "%s", "00");

//		debuglog("IMG: (data mn) pack tx %s\n", data+ol);
//		debuglog("IMG: (ammount mn) pack tx %lld\n", amount);
...
//	if(strcmp(coind->symbol, "IMG") == 0){
//		debuglog("IMG: start coinbase_create\n\n ");
//	}
	...
	char script2[32] = "526961733100"; // "7969696d7000" - "yiimp\0" in hex ascii // "526961733100" - "Rias1\0"
...
//		if(strcmp(coind->symbol, "IMG") == 0){
//			debuglog("IMG: start most recent masternodes rpc (DASH, SIB, MUE, DSR, GBX...)\n ");
//		}
...
//			if(strcmp(coind->symbol, "IMG") == 0){
//				debuglog("IMG: start (masternode_enabled && masternode)\n ");
//			}
			...
			if (payee && amount && started) {
				...
				if(masternode_use_p2sh)
					p2sh_pack_tx(coind, script_dests, amount, script_payee);
//				else if(strcmp(coind->symbol, "IMG") == 0)
//					job_pack_tx_IMG(coind, script_dests, amount, script_payee);
				else
					job_pack_tx(coind, script_dests, amount, script_payee);
			}
		}
		...
		if (templ->has_segwit_txs) strcat(templ->coinb2, commitment);
		if(strcmp(coind->symbol, "IMG") == 0){
			job_pack_tx_IMG(coind, templ->coinb2, available, NULL);
			strcat(templ->coinb2, script_dests);
		}else{
			strcat(templ->coinb2, script_dests);
			job_pack_tx(coind, templ->coinb2, available, NULL);
		...
		coind->reward = (double)available/100000000*coind->reward_mul;
//		if(strcmp(coind->symbol, "IMG") == 0){
//			debuglog("IMG: start (rest)\n ");
//			debuglog("%s total %u available %u\n", coind->symbol, templ->value, available);
//			debuglog("%s %d dests %s\n", coind->symbol, npayees, script_dests);
//			debuglog("%s rawTX templ->coinb1 %s\n", coind->symbol, templ->coinb1);
//			debuglog("%s rawTX templ->coinb2 %s\n", coind->symbol, templ->coinb2);
//		}
		return;
	}
...
//
//			if(strcmp(coind->symbol, "IMG") == 0){
//				debuglog("IMG: start (if(coind->hasmasternodes && coind->oldmasternodes) /* OLD DASH style */)\n ");
//			}
//
		char charity_payee[256] = { 0 };
		const char *payee = json_get_string(json_result, "payee");
	...
	strcat(templ->coinb2, "00000000"); // locktime
        ...
        if(strcmp(coind->symbol, "DEM") == 0){
                debuglog("DEM: start (rest - rest)\n ");
                strcat(templ->coinb2, "00");
        }
